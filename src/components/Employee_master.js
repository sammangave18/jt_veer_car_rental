...
const Employee_master = () => {
  ...
  //   const roleEmployees = employees.filter((emp) => emp.employeeNumber.startsWith(prefix));

  //   let lastNumber = 0;
  //   if (roleEmployees.length > 0) {
  //     lastNumber = Math.max(
  //       ...roleEmployees.map((emp) => parseInt(emp.employeeNumber.replace(prefix, ""), 10))
  //     );
  //   }
  //   const nextEmployeeNumber = `${prefix}${String(lastNumber + 1).padStart(3, "0")}`; // AD001, DR002, etc.
  ...
  const handleAddEmployee = () => {
    ...
    // if (existingEmployee) {
    //   alert("Duplicate Employee Number! Please retry.");
    //   return;
    // }
    ...
    for (const [key, value] of Object.entries(newEmployee)) {
      if (key === "profilePhoto" && value) {
        ...
        formData.append(key, value); // Append regular fields as text
      }
    }
  ...
  const updateEmployee = () => {
    if (!editingEmployee || !editingEmployee._id) {
      console.error(
        "Error: editingEmployee is undefined or missing _id",
        editingEmployee
      );
  ...
  return (
    ...
    <div className="container my-4">
      ...
      <div className="d-flex justify-content-between mb-3">
        ...
        <button className="btn btn-primary btn-sm mx-2" onClick={fetchEmployees}>
          🔄 Refresh
        </button>
      </div>
